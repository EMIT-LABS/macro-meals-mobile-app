...
// Types for Adjust Goals Flow values. You may extend/minimize if needed
// (Mirrors GoalsFlowState, except step/array lengths)
type AdjustGoalsFlowState = {
  majorStep: number;
...
export const useAdjustGoalsFlowStore = create<AdjustGoalsFlowState>(
  (set, get) => ({
    gender: null,
    setGender: gender => set({ gender }),
    majorStep: 0,
    subSteps: { 0: 0, 1: 0, 2: 0 },
    completed: { 0: Array(4).fill(false), 1: Array(3).fill(false), 2: [false] },
    setMajorStep: step => set({ majorStep: step }),
    setSubStep: (major, sub) =>
      set(state => ({ subSteps: { ...state.subSteps, [major]: sub } })),
    ...
    setDietaryPreference: preference => set({ dietaryPreference: preference }),
    resetSteps: () =>
      set({
        majorStep: 0,
        subSteps: { 0: 0, 1: 0, 2: 0 },
    ...
    resetToHeightMetrics: () =>
      set({ majorStep: 0, subSteps: { 0: 0, 1: 0, 2: 0 } }),
    handleBackNavigation: () => {
      ...
      if (currentSubStep > 0) {
        set(state => ({
          subSteps: {
            ...state.subSteps,
            [currentMajorStep]: currentSubStep - 1,
          },
        }));
    ...
    navigateToMajorStep: (step: number) => {
      ...
      if (canNavigate) {
        set({
          majorStep: step,
          subSteps: { ...state.subSteps, [step]: 0 },
        });
      }
    },
  })
